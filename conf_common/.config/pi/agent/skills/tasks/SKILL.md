...
3. For each topic with news: append log lines (dated), update `ball`/`next`/`status`. Set `last_activity` on every open topic you checked to the newest movement date you actually saw across its links - especially when that date is old, since that is what makes staleness visible. Set `prio` from the most urgent Linear ticket in `links`, and `due` from the nearest Linear due date (an overdue/today due date means `prio: 1` per the scale above); clear `due` when the deadline is gone from Linear and the user didn't set it by hand. Anything that clearly belongs to no existing topic: propose a new topic, create it if obvious.
