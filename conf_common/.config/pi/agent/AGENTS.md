...
- pi is installed at `/opt/pi-coding-agent` (docs in `/opt/pi-coding-agent/docs`); `which pi` is a `bwrap` wrapper that binds `$XDG_CONFIG_HOME/pi` and sets `PI_CODING_AGENT_DIR`
