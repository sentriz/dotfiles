...
function buildSection(): string {
  try {
    ...
    return [
      ...
      "```\n" + mcpUsage() + "\n```",
      "List a server's tools when you need it, discover the tool's schema, then call it. Pass args as key=value pairs; only use a JSON object when a value is deeply nested. Parse output with `jq`, never Python. Results are wrapped as `[{type: \"text\", text: \"<json string>\"}]`, so unwrap the text before parsing it:",
      "```\nmcp server linear call list_issues limit=10 query=\"foo bar\" | jq -r '.[0].text' | jq -r '.[].title'\n```",
