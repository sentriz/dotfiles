...
- A fallback that survives that bar is worth a short comment saying which case it covers, since the reader otherwise cannot tell it from a defensive default. Mirroring another tool's path logic is the usual example: name the tool and the source file, so the next reader can check it still holds.
- Identifiers are `snake_case`, but function names are `kebab-case` (`need-vault`), so they read like the commands they sit beside.
